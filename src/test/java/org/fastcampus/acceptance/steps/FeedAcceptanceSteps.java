...
public class FeedAcceptanceSteps {
    ...
    public static Long requestCreatePost(CreatePostRequestDto dto) {
        return RestAssured
            .given().log().all().body(dto).contentType(MediaType.APPLICATION_JSON_VALUE)
            .when().post("/post")
            .then().log().all().extract()
            .jsonPath().getObject("value", Long.class);
    }
    ...
    public static List<GetPostContentResponseDto> requestFeedList(Long userId) {
        return RestAssured
            ...
            .then().log().all().extract()
            .jsonPath().getList("value", GetPostContentResponseDto.class);
    }
}
